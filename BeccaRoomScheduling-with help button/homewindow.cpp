...
int getDaysBitmask(const QString &daysStr) {
    int byWeekday = 0;
    if (daysStr == "Everyday") return 127;
      ...
      , ui(new Ui::HomeWindow)
{
    ...
    // When start time changes, push end time forward if it's now before start
    connect(ui->startTimeEdit, &QTimeEdit::timeChanged, this, [this](const QTime &t) {
        if (ui->endTimeEdit->time() <= t)
            ui->endTimeEdit->setTime(t.addSecs(1800)); // bump end to start + 30 min
    });

    // When end time changes, push start time back if it's now after end
    ...
    connect(ui->frequencyComboBox, QOverload<int>::of(&QComboBox::currentIndexChanged), this, [this](int index){
    ...
    // When start date changes, push end date forward if it's now before start
    connect(ui->startDateEdit, &QDateEdit::dateChanged, this, [this](const QDate &d) {
        if (ui->endDateEdit->date() < d)
            ui->endDateEdit->setDate(d);
    });

    // When end date changes, push start date back if it's now after end
    ...
    ui->reservationTable->setHorizontalHeaderLabels({"ID", "Name", "Room", "Start Date", "Start Time", "End Time", "End Date", "Frequency", "Days"});
    ...
    connect(ui->btnPrevWeek, &QPushButton::clicked, this, [this]() {
        ui->scheduleDateEdit->setDate(ui->scheduleDateEdit->date().addDays(-7));
    });
    connect(ui->btnNextWeek, &QPushButton::clicked, this, [this]() {
        ui->scheduleDateEdit->setDate(ui->scheduleDateEdit->date().addDays(7));
    });
...
bool HomeWindow::setupDatabase()
{
    ...
    // Clean up any orphaned slots whose reservation was deleted without cascade
    QSqlQuery cleanupQuery;
    cleanupQuery.exec("DELETE FROM slots WHERE reservation_id NOT IN (SELECT id FROM reservations)");
    ...
    if (reply == QMessageBox::Yes) {
        ...
        if (Database::Room::insert(fullRoomName, error)) {
            ...
            if (error.contains("UNIQUE constraint failed", Qt::CaseInsensitive))
                QMessageBox::warning(this, "Room Already Exists",
                    "A room named \"" + fullRoomName + "\" already exists.\n"
...
void HomeWindow::on_btnDeleteRoom_clicked()
{
    ...
    QStringList existingRooms = Database::Room::retrieve();
    if (!existingRooms.contains(fullRoomName)) {
        QMessageBox::warning(this, "Room Not Found",
            "No room named \"" + fullRoomName + "\" was found.\n"
            "Please check the name and try again.");
        return;
    }

    // Check if any reservations are using this room
    QSqlQuery checkQuery;
    checkQuery.prepare("SELECT COUNT(*) FROM reservations WHERE room = :room");
    checkQuery.bindValue(":room", fullRoomName);
    if (checkQuery.exec() && checkQuery.next()) {
        int count = checkQuery.value(0).toInt();
        if (count > 0) {
            QMessageBox::StandardButton reply = QMessageBox::question(
                this, "Room In Use",
                QString("Room %1 has %2 reservation(s) assigned to it.\n\n"
                        "Deleting this room will also permanently delete all of its reservations and occurrences.\n\n"
                        "Are you sure you want to continue?")
                    .arg(fullRoomName).arg(count),
                QMessageBox::Yes | QMessageBox::Cancel);

            if (reply != QMessageBox::Yes) return;

            // Delete all reservations (slots cascade automatically)
            QSqlQuery deleteRes;
            deleteRes.prepare("DELETE FROM reservations WHERE room = :room");
            deleteRes.bindValue(":room", fullRoomName);
            if (!deleteRes.exec()) {
                QMessageBox::critical(this, "Error", "Failed to delete reservations for this room.");
                return;
            }
        }
    }
...
void HomeWindow::on_btnHelpManual_clicked()
{
    HelpDialog *helpWindow = new HelpDialog(nullptr);
...
void HomeWindow::on_btnCreateReservation_clicked()
{
    ...
    // Validate: Every Week requires at least one day selected
    if (frequencyIndex == 2 && weekdayMask == 0) {
        QMessageBox::warning(this, "No Days Selected",
            "You selected \"Every Week\" but didn't check any days.\n"
    ...
    QString confirmMsg = QString("Are you sure you want to create the following reservation?\n\n"
                                 ...
                                 "Days: %8")
                             .arg(r.name, r.room,
                                  r.startDate.toString("MM/dd/yyyy"),
                                  r.startTime.toString("hh:mm AP"),
                                  r.endDate.toString("MM/dd/yyyy"),
                                  r.endTime.toString("hh:mm AP"),
                                  frequencyStr, daysStr);
    ...
    if (reply == QMessageBox::Yes) {
        QString error;
        auto result = Database::Reservation::insert(r, error);
        ...
        if (result.has_value()) {
            ...
            ui->endTimeEdit->setTime(QTime(8, 0));
            ui->frequencyComboBox->setCurrentIndex(0);

            loadReservationsFromDatabase();
        } else {
            QMessageBox::critical(this, "Could Not Create Reservation", error);
        }
    }
}
...
void HomeWindow::on_btnEditReservation_clicked()
{
    ...
    editPopup.setAvailableRooms(Database::Room::retrieve(), room);
    editPopup.setReservationData(name, room, startDate, startTime, endTime, freqText, endDate, byWeekday);
    ...
    if (editPopup.exec() == QDialog::Accepted) {
        ...
        if (reply == QMessageBox::Yes) {
            QString error;
            if (Database::Reservation::update(r, error)) {
                QMessageBox::information(this, "Success", "Reservation successfully updated.");
                loadReservationsFromDatabase();
                drawSchedule();
            } else {
                QMessageBox::critical(this, "Conflict", error);
            }
        }
    }
}
...
void HomeWindow::on_btnDeleteReservation_clicked()
{
    ...
    QString confirmMsg = QString("Are you sure you want to permanently delete this reservation?\n\n"
                                 ...
                                 "Frequency: %7\n"
                                 "Days: %8")
                             .arg(resName, room, startDate, startTime, endDate, endTime, frequency, daysStr);
...
void HomeWindow::applyFilters()
{
    ...
    for (int i = 0; i < ui->reservationTable->rowCount(); ++i) {
        ...
        if (checkDate && showRow) {
            ...
            reservation.startDate = QDate::fromString(ui->reservationTable->item(i, 3)->text(), "MM/dd/yyyy");
            reservation.startTime = QTime::fromString(ui->reservationTable->item(i, 4)->text(), "hh:mm AP");
            reservation.endTime = QTime::fromString(ui->reservationTable->item(i, 5)->text(), "hh:mm AP");
            reservation.endDate = QDate::fromString(ui->reservationTable->item(i, 6)->text(), "MM/dd/yyyy");

            QString freqText = ui->reservationTable->item(i, 7)->text();
            if (freqText == "Does not repeat") reservation.frequency = 0;
            else if (freqText == "Everyday") reservation.frequency = 1;
            else if (freqText == "Every Week") reservation.frequency = 2;
            else if (freqText == "Every Month") reservation.frequency = 3;
            else if (freqText == "Every Year") reservation.frequency = 4;
            else reservation.frequency = 0;

            reservation.byWeekday = getDaysBitmask(ui->reservationTable->item(i, 8)->text());

            if (!isReservationOnDate(reservation, targetDate)) {
                showRow = false;
            }
        }
...
void HomeWindow::on_filterRoomComboBox_currentTextChanged(const QString &)
{
    applyFilters();
}
...
void HomeWindow::on_filterDateEdit_dateChanged(const QDate &)
{
    applyFilters();
}
...
void HomeWindow::showEvent(QShowEvent *event)
{
    ...
    skipFilter->onSkip = [=]() {
        ...
        if (pSkipFilter) pSkipFilter->deleteLater(); 
        targetLabel->show();
    };
    qApp->installEventFilter(skipFilter); 
    ...
    // =========================================================================
    // 1. SHARED MONITOR DRAWING LOGIC (Moved up so both timers can use it)
    // =========================================================================
    auto drawComputer = [textW](QPainter &painter) {
        ...
        painter.setBrush(QColor(0x2C, 0x2C, 0x2E));
        painter.setPen(Qt::NoPen);
        painter.drawRoundedRect(cpuX, cpuY, cpuW, cpuH, 12, 12);
        painter.setBrush(QColor(0x34, 0xC7, 0x59));
        ...
        painter.setBrush(QColor(0xAE, 0xAE, 0xB2));
    ...
    // We instantiate decodeTimer first so loginTimer can reference it
    QTimer *decodeTimer = new QTimer(animState); 
    decodeTimer->setProperty("revealIdx", 0);
    decodeTimer->setProperty("tick", 0);

    // =========================================================================
    // 2. PHASE 0: GREEN LOGIN ANIMATION
    // =========================================================================
    QTimer *loginTimer = new QTimer(animState);
    loginTimer->setProperty("tick", 0);

    connect(loginTimer, &QTimer::timeout, animState, [=, this]() {
        if (!pAnimState || !pAnimLabel) return;
        
        int tick = loginTimer->property("tick").toInt();
        loginTimer->setProperty("tick", tick + 1);
        ...
        // Draw Login UI inside monitor
        QFont loginFont = compFont;
        loginFont.setPointSizeF(16.0); // Slightly smaller to ensure it fits
        p.setFont(loginFont); 
        
        QColor hackerGreen("#34C759");
        p.setPen(QPen(hackerGreen, 2));
        
        // Input Boxes (Shifted Y coordinates to prevent overlap)
        p.drawRect(-160, -75, 320, 35); // Username Box
        p.drawRect(-160, -5, 320, 35);  // Password Box
        
        // Button Click Animation (Delayed to tick 110)
        bool clickPhase = (tick > 110);
        if (clickPhase) p.setBrush(hackerGreen);
        else p.setBrush(Qt::NoBrush);
        
        // Login Button
        p.drawRect(-60, 50, 120, 35);

        // Text Labels
        p.setPen(hackerGreen);
        p.drawText(QRectF(-160, -100, 320, 20), Qt::AlignLeft | Qt::AlignBottom, "USERNAME:");
        p.drawText(QRectF(-160, -30, 320, 20), Qt::AlignLeft | Qt::AlignBottom, "PASSWORD:");

        QString targetUser = "Becca Kilponen";
        QString targetPass = "************";

        // Typing Username 
        if (tick > 5) {
            int userChars = qMin(targetUser.length(), (int)((tick - 5) / 3.0));
            p.drawText(QRectF(-150, -75, 300, 35), Qt::AlignVCenter | Qt::AlignLeft, targetUser.left(userChars));
        }

        // Typing Password 
        if (tick > 60) {
            int passChars = qMin(targetPass.length(), (int)((tick - 60) / 3.0));
            p.drawText(QRectF(-150, -5, 300, 35), Qt::AlignVCenter | Qt::AlignLeft, targetPass.left(passChars));
        }

        // Button Label
        p.setPen(clickPhase ? Qt::black : hackerGreen);
        p.drawText(QRectF(-60, 50, 120, 35), Qt::AlignCenter, "LOGIN");

        p.restore();
        pAnimLabel->setPixmap(frame);

        // Transition to Decoding Phase 
        if (tick > 120) { 
            loginTimer->stop();
            loginTimer->deleteLater();
            decodeTimer->start(30); 
        }
    });

    // =========================================================================
    // 3. PHASE 1: MATRIX DECODE
    // =========================================================================
    QString gibberish = "ABCDEFGHJKLNOPQRSTUVXYZ023456789#$";

    connect(decodeTimer, &QTimer::timeout, animState, [=, this]() {
        ...
        p.save();
        p.translate(w/2.0, h/2.0);
        drawComputer(p); // Now seamlessly uses shared hardware logic!
        p.restore();
        ...
        if (rIdx > targetStr.length()) {
            ...
            QVariantAnimation *zoomAnim = new QVariantAnimation(animState); 
            ...
            connect(zoomAnim, &QVariantAnimation::valueChanged, animState, [=](const QVariant &value){
                ...
                if (compOpacity > 0.01) {
                    ...
                    drawComputer(pZ); // Also uses shared drawing
                    pZ.restore();
                }
                ...
                pZ.setFont(bigFont);
                pZ.setPen(QColor(0x34, 0xC7, 0x59));
            ...
            QParallelAnimationGroup *morphGroup = new QParallelAnimationGroup(animState);
    ...
    // Start Phase 0 instead of Phase 1!
    loginTimer->start(30);
}
...
void HomeWindow::drawSchedule() {
    ...
    // Build a lookup map of slot times keyed by "reservationId|yyyy-MM-dd"
    // so each drawn block uses the slot's actual time, not the base reservation time
    QHash<QString, QPair<QTime, QTime>> slotTimeMap;
    QHash<QString, QString> slotRoomMap;
    QSet<QString> existingSlotKeys;
    {
        QSqlQuery slotQ("SELECT reservation_id, date, start_time, end_time, room FROM slots");
        while (slotQ.next()) {
            QString key = QString("%1|%2")
                      .arg(slotQ.value(0).toInt())
                      .arg(slotQ.value(1).toString());
                QTime st = QTime::fromString(slotQ.value(2).toString(), "HH:mm");
                QTime et = QTime::fromString(slotQ.value(3).toString(), "HH:mm");
                slotTimeMap[key] = qMakePair(st, et);
                slotRoomMap[key] = slotQ.value(4).toString();
                existingSlotKeys.insert(key);
}
    }
    ...
    for (int i = 0; i < 7; ++i) {
        ...
        QString headerText = QString("%1\n%2").arg(daysOfWeek[i], dayDate.toString("MM/dd"));
    ...
    for (const auto& r : std::as_const(allRes)) {
    for (int dayIndex = 0; dayIndex < 7; ++dayIndex) {
        QDate currentDay = startOfWeek.addDays(dayIndex);

        if (isReservationOnDate(r, currentDay)) {
            QString slotKey = QString("%1|%2").arg(r.id).arg(currentDay.toString("yyyy-MM-dd"));

            if (!existingSlotKeys.contains(slotKey)) continue;

            QString slotRoom = slotRoomMap.value(slotKey, r.room);
            if (slotRoom != currentRoom) continue;

            QTime drawStart = slotTimeMap[slotKey].first;
            QTime drawEnd   = slotTimeMap[slotKey].second;

                int resStartH = drawStart.hour();
                int resStartM = drawStart.minute();
                int resEndH   = drawEnd.hour();
                int resEndM   = drawEnd.minute();
                ...
                QString blockText = QString("%1\n%2 - %3")
                                        .arg(r.name, drawStart.toString("h:mm AP"), drawEnd.toString("h:mm AP"));
                ...
                rectItem->onClicked = [this, currentDay](const Database::Reservation& res) {
    handleScheduleClick(res, currentDay);
};
...
void HomeWindow::handleScheduleClick(const Database::Reservation &r, const QDate &clickedDate)
{
    ...
    // Look up the actual slot time for THIS specific date
    // (may differ from the reservation's base time if it was individually edited)
    QTime displayStart  = r.startTime;
    QTime displayEnd    = r.endTime;
    QString displayRoom = r.room;
    {
        QSqlQuery slotQ;
        slotQ.prepare("SELECT start_time, end_time, room FROM slots "
                      "WHERE reservation_id = ? AND date = ? LIMIT 1");
        slotQ.addBindValue(r.id);
        slotQ.addBindValue(clickedDate.toString("yyyy-MM-dd"));
        if (slotQ.exec() && slotQ.next()) {
            displayStart = QTime::fromString(slotQ.value(0).toString(), "HH:mm");
            displayEnd   = QTime::fromString(slotQ.value(1).toString(), "HH:mm");
            displayRoom  = slotQ.value(2).toString();
        }
    }
    ...
    QString info = QString("<b>Name:</b> %1<br>"
                           ...
                           "<b>Repeats:</b> %6<br>"
                           "<b>Days:</b> %7")
                       .arg(r.name, displayRoom,
                            displayStart.toString("h:mm AP"), displayEnd.toString("h:mm AP"),
                            clickedDate.toString("MM/dd/yyyy"),
                            freqText, getDaysString(r.byWeekday));

    QMessageBox msgBox(this);
    msgBox.setWindowTitle("Reservation Details");
    msgBox.setText(info);
    QPushButton *editBtn    = msgBox.addButton("Edit This Day",        QMessageBox::ActionRole);
    QPushButton *editAllBtn = msgBox.addButton("Edit All Occurrences", QMessageBox::ActionRole);
    QPushButton *deleteBtn  = msgBox.addButton("Delete",               QMessageBox::DestructiveRole);
    msgBox.addButton(QMessageBox::Close);
    msgBox.exec();

    // --- EDIT THIS DAY ONLY (slot-level) ---
    if (msgBox.clickedButton() == editBtn) {
        EditDialog editPopup(this);
        editPopup.setAvailableRooms(Database::Room::retrieve(), displayRoom);
        editPopup.setReservationData(r.name, displayRoom, clickedDate,
                                     displayStart, displayEnd,
                                     freqText, r.endDate, r.byWeekday);

        if (editPopup.exec() == QDialog::Accepted) {
            QTime newStart   = editPopup.getUpdatedStartTime();
            QTime newEnd     = editPopup.getUpdatedEndTime();
            QString newRoom  = editPopup.getUpdatedRoom();
            QString error;
            if (Database::Slot::updateTime(r.id, clickedDate, newRoom, newStart, newEnd, error)) {
                QTimer::singleShot(0, this, [this]() { drawSchedule(); });
            } else {
                QMessageBox::critical(this, "Conflict", error);
            }
        }
    }
    // --- EDIT ALL OCCURRENCES (reservation-level, same as the reservations page) ---
    else if (msgBox.clickedButton() == editAllBtn) {
        EditDialog editPopup(this);
        editPopup.setAvailableRooms(Database::Room::retrieve(), displayRoom);
        editPopup.setReservationData(r.name, displayRoom, r.startDate,
                                     r.startTime, r.endTime,
                                     freqText, r.endDate, r.byWeekday);

        if (editPopup.exec() == QDialog::Accepted) {
            Database::Reservation updated = r;
            ...
            updated.frequency = (fStr == "Does not repeat") ? 0 :
                                (fStr == "Everyday")        ? 1 :
                                (fStr == "Every Week")      ? 2 :
                                (fStr == "Every Month")     ? 3 : 4;

            QString error;
            if (Database::Reservation::update(updated, error)) {
                loadReservationsFromDatabase();
                QTimer::singleShot(0, this, [this]() { drawSchedule(); });
            } else {
                QMessageBox::critical(this, "Conflict", error);
    ...
    // --- DELETE ---
        ...
        confirmBox.setWindowTitle("Delete Reservation");
        confirmBox.setText(QString("How would you like to delete <b>%1</b> on %2?")
                               .arg(r.name, clickedDate.toString("MM/dd/yyyy")));
        QPushButton *delDayBtn = confirmBox.addButton("Delete This Day Only",      QMessageBox::DestructiveRole);
        QPushButton *delAllBtn = confirmBox.addButton("Delete All Occurrences",    QMessageBox::DestructiveRole);
        confirmBox.addButton(QMessageBox::Cancel);
        confirmBox.exec();

        if (confirmBox.clickedButton() == delDayBtn) {
            if (Database::Slot::removeOne(r.id, clickedDate)) {
                // Check if any slots remain for this reservation
                // If not, delete the reservation record too so it
                // doesn't linger in the reservations table
                QSqlQuery countQuery;
                countQuery.prepare("SELECT COUNT(*) FROM slots WHERE reservation_id = ?");
                countQuery.addBindValue(r.id);
                if (countQuery.exec() && countQuery.next()) {
                    int remaining = countQuery.value(0).toInt();
                    if (remaining == 0) {
                        QSqlQuery delRes;
                        delRes.prepare("DELETE FROM reservations WHERE id = ?");
                        delRes.addBindValue(r.id);
                        delRes.exec();
                    }
                }
                loadReservationsFromDatabase();
                ...
                QMessageBox::critical(this, "Error", "Failed to delete this day's occurrence.");
            }
        }
        else if (confirmBox.clickedButton() == delAllBtn) {
            QSqlQuery query;
            query.prepare("DELETE FROM reservations WHERE id = :id");
...
void HomeWindow::on_btnPrintSchedule_clicked()
{
    ...
    if (dialog.exec() == QDialog::Accepted) {
        ...
        QString weekString = QString("%1 - %2").arg(startOfWeek.toString("MMM d"), endOfWeek.toString("MMM d, yyyy"));
...
{
    // 1. Only allow files named exactly "reservations.db"
    QString srcPath = QFileDialog::getOpenFileName(
        this, tr("Import Database"), "", "Database Files (*.db)");

    if (srcPath.isEmpty()) return;

    // 2. Validate the filename
    QFileInfo srcInfo(srcPath);
    if (srcInfo.fileName() != "reservations.db") {
        QMessageBox::warning(this, "Invalid File",
            "The selected file must be named \"reservations.db\".\n"
            "Please select a valid exported database file.");
        return;
    }

    // 3. Confirm with the user before overwriting
    QMessageBox::StandardButton reply = QMessageBox::question(
        this, "Confirm Import",
    ...
    // 4. Resolve the actual path the running DB file is at
    QString destPath = db.databaseName();

    // 5. Close and fully remove the Qt DB connection before touching the file
    db.close();
    ...
    if (QFile::exists(destPath)) {
        if (!QFile::remove(destPath)) {
            QMessageBox::critical(this, "Import Failed",
                "Could not remove the existing database file.\n"
                "Make sure no other program has it open.");
            // Re-open the old connection so the app still works
            db = QSqlDatabase::addDatabase("QSQLITE");
    ...
    // 7. Copy the new file into place
    if (!QFile::copy(srcPath, destPath)) {
        QMessageBox::critical(this, "Import Failed",
            "Could not copy the selected database file.\n"
    ...
    // 8. Re-open the new database
    db = QSqlDatabase::addDatabase("QSQLITE");
    db.setDatabaseName(destPath);
    if (!db.open()) {
        QMessageBox::critical(this, "Database Error",
            "Database file was replaced, but could not be opened:\n" + db.lastError().text());
        return;
    }

    // Enable foreign keys on the new connection
    QSqlQuery pragmaQuery;
    pragmaQuery.exec("PRAGMA foreign_keys = ON");

    // 9. Clear all combo boxes before reloading to prevent duplicate entries
    ui->roomComboBox->clear();
    ui->filterRoomComboBox->clear();
    ui->scheduleRoomComboBox->clear();
    ui->filterRoomComboBox->addItem("All Rooms");
    ...
    int roomCount = Database::Room::retrieve().size();
    int resCount  = Database::Reservation::retrieve().size();
    QMessageBox::information(this, "Import Successful",
        QString("Database imported successfully.\n\n"
                "Loaded %1 room(s) and %2 reservation(s).")
            .arg(roomCount).arg(resCount));
}

void HomeWindow::on_btnExportDatabase_clicked()
{
    // Use the actual path Qt is using for the database
    QString srcPath = db.databaseName();

    if (!QFile::exists(srcPath)) {
        QMessageBox::warning(this, "Export Failed",
            "The database file could not be found at:\n" + srcPath);
        return;
    }

    // Ask user where to save it, pre-filling the filename
    QString destPath = QFileDialog::getSaveFileName(
        this, tr("Export Database"),
        QDir::homePath() + "/reservations.db",
        "Database Files (*.db)");

    if (destPath.isEmpty()) return;

    // If the user didn't type the .db extension, add it
    if (!destPath.endsWith(".db", Qt::CaseInsensitive))
        destPath += ".db";
    ...
    if (QFile::exists(destPath)) {
        if (!QFile::remove(destPath)) {
            QMessageBox::critical(this, "Export Failed",
                "A file already exists at that location and could not be replaced.");
            return;
        }
    }
    ...
    if (QFile::copy(srcPath, destPath)) {
        ...
        QMessageBox::critical(this, "Export Failed",
            "Could not copy the database file.\n"
